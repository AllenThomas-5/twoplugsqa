...
public class TP_010_Bid_Service {
	WebDriver driver;
	String rootPath;
	...
	@Test(priority=4)
	public void clickOnNegotiate() throws InterruptedException {
		
		Boolean isNegotiate = false;
		
		String expected_title = "SUBMIT YOUR BID";
		
		//Select the Negotiate Button
		WebElement negotiate_button_element = driver.findElement(By.xpath("//section[@class='viewService detailsSect serviceTheme']//div//div[@class='text-right row']//div//a[@id='bid_button']"));
		negotiate_button_element.click();
		
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(2));
		
		// Check the Bid Modal Title
		WebElement bid_modal_element = driver.findElement(By.xpath("//div[@id='myModalCatering']//div//div//div//h4[@class='modal-title']"));
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(2));
